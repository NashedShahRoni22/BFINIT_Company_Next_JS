...
function SectionBadge({ icon: Icon, label }) {
  return (
    <div className="mx-auto inline-flex items-center gap-2 rounded-full border border-gray-200/60 bg-white px-4 py-2 text-sm font-medium shadow-sm">
      <span className="flex items-center justify-center rounded-full bg-black/85 p-1">
    ...
    <li className="flex items-start gap-3">
      <span
        className={`mt-0.5 flex size-7 shrink-0 items-center justify-center rounded-full ${
          accent ? "bg-brand/10 text-brand" : "bg-secondary/30 text-dark/70"
        }`}>
...
function PillarCard({ icon: Icon, title, subtitle, color }) {
  return (
    <div
      ...
      <div className="flex size-12 items-center justify-center rounded-xl bg-dark/5 text-dark transition-colors duration-300 group-hover:bg-brand group-hover:text-white">
...
export default function ScottyPumpkinPage() {
  return (
    <main className="w-full bg-linear-to-b from-[#e6f9f3] to-[#f5fbff]">
      {/* ── HERO ─────────────────────────────── */}
      <section className="flex min-h-[60vh] w-full flex-col justify-center px-2 py-20 md:py-32">
        <Container>
          <div className="flex flex-col items-center text-center text-black">
            <SectionBadge
              icon={LuGlobe}
              label="Scotty Pumpkin Web3 Ecosystem"
            />
            ...
            <div className="mt-10 flex flex-col items-center gap-4 sm:flex-row">
              <Link
                ...
                className="group flex items-center rounded-lg bg-brand px-6 py-3 font-medium text-white transition-all duration-300 hover:bg-brand/80 hover:text-dark">
                Get Started
                <LuArrowRight className="ml-2 size-4 transition-transform duration-200 group-hover:translate-x-1" />
              </Link>
              <Link
                href="/scotty-pumpkin-marketplace"
                className="rounded-lg border border-gray-300 bg-white px-6 py-3 font-medium text-gray-700 transition-all duration-200 hover:border-brand hover:bg-brand/5 hover:text-brand">
                Visit Marketplace
              </Link>
            </div>
          </div>
            ...
            <div className="grid gap-0 lg:grid-cols-2">
              ...
              <div className="border-b border-gray-100 p-8 lg:border-b-0 lg:border-r">
                <div className="mb-2 inline-flex items-center gap-2 rounded-full bg-secondary/30 px-3 py-1 text-xs font-semibold uppercase tracking-widest text-dark/60">
                  ...
                  <h2 className="font-inter">Powered by Scotty Pumpkin Web3</h2>
                </div>
                ...
                <ul className="mt-6 space-y-4">
                  <FeatureRow
                    icon={LuShoppingCart}
                    text="Build and manage your ecommerce website"
                  />
                  <FeatureRow
                    icon={LuPackage}
                    text="Sell products and services online"
                  />
                  <FeatureRow
                    icon={LuMegaphone}
                    text="Market your business globally"
                  />
                </ul>
              </div>
              ...
              <div className="p-8">
                <div className="mb-2 inline-flex items-center gap-2 rounded-full bg-brand/10 px-3 py-1 text-xs font-semibold uppercase tracking-widest text-brand">
                ...
                <ul className="mt-6 space-y-4">
                  <FeatureRow
                    icon={LuCoins}
                    text="Accept SPUMP payments in your Bfinit shop"
                    accent
                  />
                  <FeatureRow
                    icon={LuShoppingBag}
                    text="Enable Web3 checkout"
                    accent
                  />
                  <FeatureRow
                    icon={LuLock}
                    text="Access secure escrow payment options"
                    accent
                  />
                  <FeatureRow
                    icon={LuStore}
                    text="Connect your store to the Scotty Pumpkin Marketplace"
                    accent
                  />
                  <FeatureRow
                    icon={LuShield}
                    text="Operate within a secure and integrated Web3 ecosystem"
                    accent
                  />
                </ul>
              </div>
      ...
      <section className="px-2 py-12">
        <Container>
          <div className="relative overflow-hidden rounded-2xl bg-black px-8 py-10 text-white md:px-12">
            {/* Decorative blobs */}
            <div className="pointer-events-none absolute -right-10 -top-10 size-48 rounded-full bg-secondary opacity-10 blur-3xl" />
            <div className="pointer-events-none absolute -bottom-10 -left-10 size-48 rounded-full bg-brand opacity-10 blur-3xl" />
            ...
            <div className="relative flex flex-col items-start gap-4 md:flex-row md:items-center md:justify-between">
              ...
              <Link
                href="/bfinit-ecomerce-platform"
                className="group flex shrink-0 items-center rounded-lg bg-secondary px-6 py-3 font-medium text-black transition-all duration-300 hover:bg-white">
                Start Building
                <LuArrowRight className="ml-2 size-4 transition-transform duration-200 group-hover:translate-x-1" />
              </Link>
            </div>
        ...
        <Container>
          <div className="mx-auto max-w-3xl text-center">
            <SectionBadge
              ...
              label="Scotty Pumpkin Eco Market System"
            />
            <h2 className="mt-6 font-sora text-3xl font-bold md:text-4xl font-inter">
            ...
            <p className="mt-5 leading-relaxed text-dark/70 md:text-lg">
              ...
              <strong className="text-dark">E-Bfinit</strong> (Ecommerce
              Builder),{" "}
            ...
            {[
              {
                ...
                desc: "End-to-end payment security across all platforms",
              },
              {
                icon: LuShield,
                title: "Escrow Protection",
                desc: "Trustless escrow mechanisms for every transaction",
              },
              {
                icon: LuUsers,
                title: "Community-Driven",
                desc: "Infrastructure backed and governed by the community",
              },
